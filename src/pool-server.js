...
const ConsoleMonitor = require('./lib/console-monitor'); // 콘솔 모니터링 추가
...
class WorldLandPoolServer {
    ...
    async setupStratum() {
        try {
            ...
            // 콘솔 모니터 조건부 시작
            if (process.env.DISABLE_CONSOLE_MONITOR !== 'true') {
                this.modules.consoleMonitor = new ConsoleMonitor(this.stratumServer);
                console.log('📺 콘솔 모니터링 시스템 활성화');
            } else {
                this.modules.consoleMonitor = null;
                console.log('📺 콘솔 모니터링 시스템 비활성화 (환경변수 설정)');
            }
    ...
    async sendRealtimeUpdate(socket = null) {
        try {
            ...
            // ⭐ Stratum 서버의 실시간 통계를 우선 사용
            const realtimeStats = {
                ...stats,
                // Stratum 서버의 메모리 통계를 DB 통계보다 우선 사용
                total_hashrate: stats?.total_hashrate || 0,
                miners_count: connectedMiners.length,
                daily_valid_shares: stratumStats.validShares || 0,
                daily_invalid_shares: stratumStats.invalidShares || 0,
                daily_total_shares: stratumStats.sharesSubmitted || 0,
                blocks_found_today: stratumStats.blocksFound || stats?.blocks_found_today || 0
            };
            ...
            const updateData = {
                type: 'statsUpdate',
                payload: {
                    poolStats: realtimeStats || {},  // ⭐ 수정: stats → realtimeStats
                    activeMiners: miners || [],
            ...
            try {
                ...
                res.json({
                    ...
                    stats: {
                        ...
                        shares: {
                            valid: stats?.daily_valid_shares || 0,
                            invalid: stats?.daily_invalid_shares || 0,
                            total: stats?.daily_total_shares || 0,
                            acceptance_rate: stats?.daily_total_shares > 0 ? 
                                ((stats.daily_valid_shares / stats.daily_total_shares) * 100).toFixed(1) + '%' : '0%'
                        }
                    },
                    stratum: {
                        host: 'www.doldari.com',
    ...
    async start() {
        ...
        try {
            ...
            console.log(`📍 Stratum: www.doldari.com:${this.stratumServer?.port || 3333}`);
    ...
    async showServerStatus() {
        ...
        console.log(`├─ 📊 총 Share 제출: ${stratumStats.sharesSubmitted || 0}`);
        console.log(`├─ ✅ 승인된 Share: ${stratumStats.validShares || 0}`);
        console.log(`├─ ❌ 거부된 Share: ${stratumStats.invalidShares || 0}`);
        console.log(`├─ 🏆 발견된 블록: ${stratumStats.blocksFound || 0}`);
        console.log(`├─ 👥 연결된 채굴자: ${stratumStats.authorizedConnections || 0}명`);
    ...
    async shutdown() {
        ...
        try {
            ...
            // 콘솔 모니터 종료
            if (this.modules.consoleMonitor) {
                this.modules.consoleMonitor.stop();
                console.log('✅ 콘솔 모니터 종료');
            } else {
                console.log('✅ 콘솔 모니터 비활성화 상태 (종료 건너뜀)');
            }
