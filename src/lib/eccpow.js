...
class ECCPoWValidator {
    ...
    async validateSubmittedECCPoW(codewordHex, mixDigestHex, nonce, poolDifficulty, blockHeight = 0, codeLength = null) {
        try {
            ...
            // 유효성 판단
            // 클라이언트와 동일한 적응형 임계값 적용
            let adaptiveThreshold = threshold;

            // 최근 성과에 따른 미세 조정 (클라이언트와 동일)
            // 간단한 성공률 추정 (실제 구현에서는 더 정교한 통계 필요)
            const estimatedSuccessRate = 0.005; // 기본값 0.5%

            if (estimatedSuccessRate < 0.001) { // 성공률이 너무 낮으면 5% 완화
                adaptiveThreshold = Math.floor(threshold * 1.05);
            } else if (estimatedSuccessRate > 0.01) { // 성공률이 높으면 5% 강화
                adaptiveThreshold = Math.floor(threshold * 0.95);
            }

            // 유효성 판단
            const isValid = weight <= adaptiveThreshold;
