    ...
    async getPoolStats() {
        try {
            const [rows] = await this.pool.execute(`
                SELECT 
                    ps.*,
                    (SELECT COUNT(*) FROM shares WHERE is_valid = TRUE AND submitted_at > CURDATE()) as daily_valid_shares,
                    (SELECT COUNT(*) FROM shares WHERE is_valid = FALSE AND submitted_at > CURDATE()) as daily_invalid_shares,
                    (SELECT COUNT(*) FROM shares WHERE submitted_at > CURDATE()) as daily_total_shares
                FROM pool_stats ps WHERE stat_type = 'current'
            `);
            ...
            if (rows.length === 0) {
                ...
                const [newRows] = await this.pool.execute(`
                    SELECT 
                        ps.*,
                        0 as daily_valid_shares,
                        0 as daily_invalid_shares, 
                        0 as daily_total_shares
                    FROM pool_stats ps WHERE stat_type = 'current'
                `);
                return newRows[0];
            }
