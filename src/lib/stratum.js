...
class WorldLandStratumServer extends EventEmitter {
    constructor(config) {
        ...
        this.logLevel = 4; // 0=error, 1=warn, 2=info, 3=debug, 4=trace (모든 로그)
        ...
        // 환경변수로 로깅 레벨 제어
        this.logLevel = parseInt(process.env.STRATUM_LOG_LEVEL) || 4; // 기본값 4 (모든 로그)
        this.forceConsoleOutput = process.env.FORCE_CONSOLE_OUTPUT !== 'false'; // 기본값 true

        console.log(`🔧 Stratum 로깅 설정: 레벨=${this.logLevel}, 강제출력=${this.forceConsoleOutput}`);
    }
    ...
    async handleSubmit(connection, params, id) {
        ...
        console.log('\n' + '='.repeat(80));
        console.log(`🔍 Share 제출 수신: ${new Date().toLocaleTimeString()}`);

        console.log(`⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰\n`);
        console.log(`⏰⏰⏰handleSubmit function started.⏰⏰⏰\n`);
        console.log(`⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰⏰\n`);
        
        // WorldLand ECCPoW 확장 프로토콜 지원 (클라이언트 ToStratumParams 순서와 일치)
        let addressWithWorker, jobId, extraNonce2, nTime, nonce, codeword, mixDigest, codeLength;
        
        console.log(`params.length: ${params.length}\n`);
        console.log(`params: ${params}\n`);
        if (params.length >= 8) {
            // ECCPoW 확장 프로토콜 (8개 파라미터)
            // 순서: WorkerName, JobID, ExtraNonce2, NTime, Nonce, Codeword, MixDigest, CodeLength
            [addressWithWorker, jobId, extraNonce2, nTime, nonce, codeword, mixDigest, codeLength] = params;
        } else if (params.length >= 7) {
            // ECCPoW 확장 프로토콜 (7개 파라미터, codeLength 없음)
            [addressWithWorker, jobId, extraNonce2, nTime, nonce, codeword, mixDigest] = params;
            codeLength = null;
        } else if (params.length >= 5) {
            // 기존 표준 프로토콜 (5개 파라미터)
            [addressWithWorker, jobId, extraNonce2, nTime, nonce] = params;
            codeword = null;
            mixDigest = null;
            codeLength = null;
        } else {
            console.log(`⏰⏰⏰Invalid parameter count.⏰⏰⏰\n`);
            this.sendError(connection, id, -1, 'Invalid parameter count');
            return;
        }

        console.log(`addressWithWorker: ${addressWithWorker} \n`);
        ...
        if (addressWithWorker && addressWithWorker.includes('.')) {
        ...
        console.log(`제출된 주소: ${cleanAddress} \n`);
        console.log(`workerName: ${workerName} \n`);
        
        // 주소 유효성 검사
        if (!this.isValidAddress(cleanAddress)) {
            console.log(`❌ 검증 실패: 잘못된 주소 형식`);
            console.log(`   - 제출된 주소: ${cleanAddress}`);
            console.log('='.repeat(80) + '\n');
            this.sendError(connection, id, -20, 'Invalid address format');
            return;
        }
        
        // 연결 정보 업데이트 (주소가 다른 경우)
        if (connection.address !== cleanAddress) {
            connection.address = cleanAddress;
            connection.workerName = workerName;
            console.log(`🔄 연결 정보 업데이트: ${cleanAddress}.${workerName}`);
        }
        ...
        // ⭐ Share 제출 즉시 통계 업데이트 및 진행상황 추가
        this.stats.sharesSubmitted++;
        this.stats.eccpowValidations++;
        this.miningProgress.totalSubmissions++;

        // 최근 활동에 즉시 추가
        const workerId = `${cleanAddress}.${workerName}`;
        ...
        // 최근 Share는 최대 50개만 유지
        if (this.miningProgress.recentShares.length > 50) {
            this.miningProgress.recentShares = this.miningProgress.recentShares.slice(0, 50);
        }
        ...
        if (validation.valid) {
            ...
            console.log(`   🔢 레벨: ${validation.level}`);
            console.log(`   ⏰ ${new Date().toLocaleTimeString()}\n`);
        } else {
            console.log('\n⚠️ ECCPoW 검증 실패 ⚠️');
            console.log(`   ❌ 사유: ${validation.error}`);
            console.log(`   ⏰ ${new Date().toLocaleTimeString()}\n`);
        }
        ...
        // 7단계: Share 처리 (통계는 이미 위에서 업데이트했으므로 중복 방지)
        if (validation.valid) {
            // processValidShare에서 sharesSubmitted 중복 업데이트 방지
            await this.processValidShare(connection, job, validation, shareKey, workerName, id);
        } else {
            // processInvalidShare에서 sharesSubmitted 중복 업데이트 방지
    ...
    async processValidShare(connection, job, validation, shareKey, workerName, id) {
        // sharesSubmitted는 handleSubmit에서 이미 증가시켰으므로 여기서는 제거
        this.stats.validShares++;
        this.miningProgress.validSubmissions++;
        connection.validShares++;
        
        // 최근 Share 상태 업데이트
        const workerId = `${connection.address}.${workerName}`;
        const recentShare = this.miningProgress.recentShares.find(
            share => share.worker === workerId && share.status === 'pending'
        );
        if (recentShare) {
            recentShare.valid = true;
            recentShare.weight = validation.weight;
            recentShare.networkType = validation.networkType;
            recentShare.status = 'accepted';
        }
        ...
        // 🎉 Share 승인 축하 메시지 (강제 출력)
        console.log('\n' + '🎉'.repeat(50));
        console.log('🎉 Share 승인됨! 🎉');
        console.log('🎉'.repeat(50));
        console.log(`✅ Share 승인 성공!`);
        console.log(`   - 승인 번호: #${this.stats.validShares}`);
        console.log(`   - 워커: ${connection.address}.${connection.workerName}`);
        ...
        // 로거를 통한 추가 로깅
        this.logInfo(`🎉 Share 승인!`, {
            worker: `${connection.address}.${connection.workerName}`,
            shareNumber: this.stats.validShares,
            successRate: ((this.stats.validShares / this.stats.sharesSubmitted) * 100).toFixed(1) + '%'
        });
    ...
    async processInvalidShare(connection, job, validation, workerName, id) {
        ...
        this.miningProgress.invalidSubmissions++;
        ...
        if (recentShare) {
            recentShare.valid = false;
            recentShare.weight = validation.weight || 0;
            recentShare.networkType = validation.networkType;
            recentShare.status = 'rejected';
            recentShare.error = validation.error;
        }
        ...
        // 로거를 통한 추가 로깅
        this.logWarn(`❌ Share 거부!`, {
            worker: `${connection.address}.${connection.workerName}`,
            reason: validation.error || 'ECCPoW 검증 실패',
            failureRate: ((this.stats.invalidShares / this.stats.sharesSubmitted) * 100).toFixed(1) + '%'
        });
    ...
    async recordValidShare(connection, job, validation, workerName) {
        console.log(`🔍 Share 기록 시작: ${connection.address}.${workerName}`);
            ...
            await new Promise((resolve, reject) => {
                this.server.listen(this.port, this.host, (error) => {
                    if (error) reject(error);
                        ...
                        // 🔥 서버 시작 확인 로그
                        console.log('\n' + '🚀'.repeat(60));
                        console.log(`🚀 Stratum 서버 시작 완료! 🚀`);
                        console.log('🚀'.repeat(60));
                        console.log(`📍 주소: ${this.host}:${this.port}`);
                        console.log(`⚡ 알고리즘: ECCPoW`);
    ...
    async validateShare(job, nonce, workerAddress, blockHeight, eccpowData = null) {
        try {
            ...
            // 네트워크 타입 강제로 Seoul 설정 (클라이언트와 동일)
            const networkType = 'seoul';
            ...
            if (eccpowData && eccpowData.codeword && eccpowData.mixDigest) {
                ...
                validation.networkType = networkType; // 강제 설정
            ...
            return { valid: false, error: error.message, networkType: networkType };
        }
    }
    ...
    async handleAuthorize(connection, params, id) {
        ...
        // 강제 콘솔 출력
        console.log('\n🔐 채굴자 인증 완료! 🔐');
        console.log(`   👤 워커: ${cleanAddress}.${workerName}`);
        console.log(`   🌐 IP: ${connection.ip}`);
        console.log(`   📊 총 연결: ${this.stats.authorizedConnections}명`);
        console.log(`   ⏰ ${new Date().toLocaleTimeString()}\n`);
    }
    ...
    async sendJob(connection) {
        ...
        // mining.notify 전송 (클라이언트 Job 구조체 필드명 사용)
    ...
    // ===============================
    // 데이터베이스 기록 메서드들 (누락된 부분)
    // ===============================
    async recordValidShare(connection, job, validation, workerName) {
        try {
            if (!this.dbPromise) {
                console.error('❌ 데이터베이스 연결이 설정되지 않음');
                return;
            }

            const miner = await dbManager.getOrCreateMiner(connection.address, workerName);
    ...
    handleConnection(socket) {
        ...
        console.log(`🔌 새로운 연결! 🔌`);
        console.log('🔌'.repeat(60));
        console.log(`📍 IP 주소: ${connection.ip}`);
        console.log(`🆔 연결 ID: ${connectionId}`);
        console.log(`📊 총 연결 수: ${this.stats.activeConnections}`);
    ...
    handleData(connection, data) {
        ...
        for (const message of messages) {
            ...
            // 🔍 들어오는 모든 Stratum 메시지 디스플레이
            console.log('\n' + '📨'.repeat(60));
            console.log(`📨 [${new Date().toLocaleTimeString()}] 클라이언트 메시지 수신`);
            console.log('📨'.repeat(60));
            console.log(`🌐 IP: ${connection.remoteAddress || connection.ip}`);
            console.log(`🆔 연결 ID: ${connection.id}`);
            console.log(`👤 워커: ${connection.address ? `${connection.address}.${connection.workerName}` : '미인증'}`);
            console.log(`📏 메시지 길이: ${message.length} bytes`);
            console.log(`📄 원본 메시지:`);
            console.log(`   ${message}`);
            ...
            try {
                ...
                console.log(`   - ID: ${request.id || 'N/A'}`);
                console.log(`   - JSON-RPC: ${request.jsonrpc || '없음'}`);
                console.log(`   - Params 개수: ${request.params ? request.params.length : 0}`);
                
                if (request.params && request.params.length > 0) {
                    console.log(`   - Params:`);
                    request.params.forEach((param, index) => {
                        if (typeof param === 'string' && param.length > 50) {
                            console.log(`     [${index}]: ${param.substring(0, 50)}... (${param.length} chars)`);
                        } else {
    ...
    async handleRequest(connection, request) {
        ...
        console.log(`handleRequest function started\n`);

        // JSON-RPC 2.0 호환성 확인
        if (jsonrpc && jsonrpc !== "2.0") {
            this.sendError(connection, id, -32600, 'Invalid JSON-RPC version');
            return;
        }
        ...
        try {
            switch (method) {
                ...
                case 'mining.ping':
                    await this.handlePing(connection, params, id);
                    break;
                    
                default:
                    this.sendError(connection, id, -32601, 'Method not found');
            }
        } catch (error) {
            this.logError(`요청 처리 오류 [${method}]`, { error: error.message });
            this.sendError(connection, id, -32603, 'Internal error');
        }
    }

    // Keep-alive ping 처리
    async handlePing(connection, params, id) {
        // 활동 시간 업데이트
        connection.lastActivity = Date.now();
        
        // 연결 추적 업데이트
        this.connectionTracker.updateActivity(connection.id, 'PING_RECEIVED', {});
        
        this.logDebug(`💓 Keep-alive ping 수신`, {
            ip: connection.ip,
            worker: connection.address ? `${connection.address}.${connection.workerName}` : '미인증'
        });
    ...
    async handleSubscribe(connection, params, id) {
        ...
        // 로그에 JSON-RPC 2.0 형식임을 명시
        this.logDebug(`📝 구독 완료 (JSON-RPC 2.0)`, {
            ip: connection.ip,
    ...
    createJobForConnection(connection) {
        ...
        // 클라이언트 Job 구조체와 정확히 매칭
        return {
            // 클라이언트 Job 구조체 필드들
            JobID: jobId,
            PrevHash: template.previousblockhash || '0'.repeat(64),
            CoinbasePart1: this.generateCoinbasePart1(connection),
            CoinbasePart2: this.generateCoinbasePart2(),
            MerkleBranch: [],
            Version: template.version || '0x20000000',
            NBits: template.bits || '0x1d00ffff',
            NTime: (template.curtime || Math.floor(Date.now() / 1000)).toString(16),
            CleanJobs: true,
            ExtraNonce1: connection.extraNonce1 || this.generateExtraNonce1(),
            ExtraNonce2Size: 4,
            ...
            createdAt: Date.now(),
            minerAddress: connection.address
        };
    }

    // 새로 추가할 메서드들 (createJobForConnection 메서드 다음에 추가)

    // Coinbase Part1 생성
    generateCoinbasePart1(connection) {
        // 간단한 coinbase part1 생성 (실제로는 더 복잡한 구조 필요)
        const blockHeight = this.currentBlockTemplate?.height || 0;
        const heightBuffer = Buffer.alloc(4);
        heightBuffer.writeUInt32LE(blockHeight, 0);
        
        // 기본 coinbase 구조
        const version = Buffer.from('01000000', 'hex'); // version 1
        const inputCount = Buffer.from('01', 'hex'); // 1 input
        const prevHash = Buffer.alloc(32, 0); // null hash
        const prevIndex = Buffer.from('ffffffff', 'hex'); // coinbase input
        
        const scriptSigStart = Buffer.concat([
            Buffer.from([heightBuffer.length]), // script length prefix
            heightBuffer
        ]);
    ...
    // Coinbase Part2 생성  
    generateCoinbasePart2() {
        // ExtraNonce 공간 이후의 coinbase 부분
        const sequence = Buffer.from('ffffffff', 'hex');
        const outputCount = Buffer.from('01', 'hex'); // 1 output
        
        // 4.0 WLC 보상 (wei 단위)
        const rewardValue = Buffer.alloc(8);
        rewardValue.writeBigUInt64LE(BigInt('4000000000000000000'), 0);
        
        // 간단한 P2PKH 출력 스크립트
        const scriptPubKey = Buffer.from('1976a914' + '0'.repeat(40) + '88ac', 'hex');
    ...
    sendMessage(connection, message) {
        try {
            ...
            // 🔍 나가는 모든 Stratum 메시지 디스플레이
            console.log('\n' + '📤'.repeat(60));
            console.log(`📤 [${new Date().toLocaleTimeString()}] 서버 메시지 전송`);
            console.log('📤'.repeat(60));
            console.log(`🌐 IP: ${connection.remoteAddress || connection.ip}`);
            console.log(`👤 워커: ${connection.address ? `${connection.address}.${connection.workerName}` : '미인증'}`);
            console.log(`📏 메시지 길이: ${data.length} bytes`);
            console.log(`📄 전송 메시지:`);
            
            // 메시지 타입별 구분
            if (message.method) {
                console.log(`   🔔 알림: ${message.method}`);
                if (message.method === 'mining.notify') {
                    console.log(`      - Job ID: ${message.params[0]}`);
                    console.log(`      - Clean Jobs: ${message.params[8]}`);
                } else if (message.method === 'mining.set_difficulty') {
                    console.log(`      - 새 난이도: ${message.params[0]}`);
                }
            } else if (message.id !== null && message.id !== undefined) {
                if (message.error) {
                    console.log(`   ❌ 오류 응답 (ID: ${message.id})`);
                    console.log(`      - 코드: ${message.error.code}`);
                    console.log(`      - 메시지: ${message.error.message}`);
                } else {
                    console.log(`   ✅ 성공 응답 (ID: ${message.id})`);
                    console.log(`      - 결과: ${message.result}`);
                }
            }
            
            console.log(`   원본: ${data.trim()}`);
            console.log('📤'.repeat(60) + '\n');
            
            connection.socket.write(data);
        } catch (error) {
            console.log(`❌ 메시지 전송 오류: ${error.message}`);
            console.log('📤'.repeat(60) + '\n');
    ...
    getStats() {
        ...
        // 기존 메모리 기반 통계를 기본값으로 사용
        let validShares = this.stats.validShares || 0;
        let invalidShares = this.stats.invalidShares || 0;
        let totalShares = this.stats.sharesSubmitted || 0;
        
        // 데이터베이스 통계가 있다면 사용 (비동기 조회는 별도 메서드로)
        if (this.dbShareStats) {
            validShares = this.dbShareStats.validShares;
            invalidShares = this.dbShareStats.invalidShares;
            totalShares = this.dbShareStats.totalShares;
        }
        
        const shareAcceptanceRate = totalShares > 0 ? 
            ((validShares / totalShares) * 100).toFixed(2) + '%' : '0%';
    ...
    validateECCPoWData(codeword, mixDigest, codeLength) {
        try {
            ...
            // hex 형식 검증 (0x 접두사 제거 후 검증)
            const cleanCodeword = codeword.startsWith('0x') ? codeword.slice(2) : codeword;
            if (!/^[0-9a-fA-F]+$/.test(cleanCodeword)) {
                return false;
            }
            ...
            const cleanMixDigest = mixDigest.startsWith('0x') ? mixDigest.slice(2) : mixDigest;
            if (cleanMixDigest.length !== 64 || !/^[0-9a-fA-F]{64}$/.test(cleanMixDigest)) {
                return false;
            }
            
            // CodeLength 검증 강화 (클라이언트와 일치)
            if (codeLength !== undefined) {
                if (!Number.isInteger(codeLength) || codeLength <= 0) {
                    return false;
                }
                // 클라이언트와 동일한 범위 검증 추가
                if (codeLength < 64 || codeLength > 4096) {
                    return false;
                }
            }
