...
class ConsoleMonitor {
    constructor(stratumServer) {
        ...
        if (process.env.DISABLE_CONSOLE_MONITOR === 'true') {
            this.isActive = false;
            console.log('📺 콘솔 모니터링 시스템 비활성화됨 (환경변수)');
            return;
        }
    ...
    getRecentActivity() {
        // 실제 최근 Share 활동 반환
        return this.server.miningProgress.recentShares.slice(0, 3).map(share => {
            const timestamp = new Date(share.timestamp).toLocaleTimeString();
            const workerDisplay = share.worker.includes('.') ? 
                share.worker : `${share.worker.slice(0, 8)}...default`;
            const status = share.valid ? '✅ 승인' : '❌ 거부';
            const weight = share.weight ? ` (가중치: ${share.weight})` : '';
            
            return `${timestamp} - Share ${status} by ${workerDisplay}${weight}`;
        });
    }
